...
class ImageLayer extends MoveableLayer {
  ...
  render(ctx_out, ref_time) {
    ...
    if (f) {
      let scale = f[2];
      let rotation = f[3] * Math.PI / 180;
      let cx = f[0] + this.canvas.width / 2;
      let cy = f[1] + this.canvas.height / 2;
      ...
      this.ctx.drawImage(this.img, -this.width / 2 * scale, -this.height / 2 * scale, this.width * scale, this.height * scale);
      this.ctx.restore();
...
class TextLayer extends MoveableLayer {
  ...
  init(player, preview) {
    ...
    settings.add('font', null,
      select => {
        ...
        const options = ['Georgia', 'Arial', 'Monospace', 'Times New Roman'];
  ...
  render(ctx_out, ref_time) {
    ...
    if (f) {
      let scale = f[2];
      this.ctx.font = Math.floor(scale * 30) + "px " + this.font;
